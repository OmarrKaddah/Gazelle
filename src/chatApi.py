...
from db.repositories.chatRepo import createChat, createMessage, deleteChat, getChatById, listChats, listMessages
from db.repositories.memoryRepo import getChatMemory, listUserMemory, upsertChatMemory, upsertUserMemory
...
async def retrieveEndpoint(req: ChatRequest, user=Depends(getCurrentUser)):
    print(
...
async def chatStream(
    ...
    if req.chatId:
        ...
        chat = await getChatById(session, userId, chatId)
        if not chat:
            raise HTTPException(status_code=404, detail="Chat not found")
    else:
        title = req.query.strip()[:60] or "New conversation"
    ...
    async def eventGen():
        ...
        async for event in streamTokens(req.query, chunks, req.provider):
            try:
                payload = json.loads(event[6:].strip())
                if payload.get("type") == "token":
                    assistantTextParts.append(payload.get("text", ""))
            except json.JSONDecodeError:
                pass
            yield event

        assistantText = "".join(assistantTextParts)
        chatSummary = buildChatSummary(req.query, assistantText)
        ...
        async with asyncSessionFactory() as streamSession:
            await createMessage(streamSession, chatId, "assistant", assistantText, countTokens(assistantText))
            await upsertChatMemory(streamSession, chatId, chatSummary, extractedEntities, chatMetadata)
            await logAudit(streamSession, userId, "message.create", "chat", str(chatId))
            await streamSession.commit()
