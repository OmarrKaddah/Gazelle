...
- **`retrieve.py`** — `RetrievalIndex` loads all heavy state once (entity embeddings, entity-entity adjacency, entity→chunk map) so per-query cost is ~300 ms. Edge layers are toggled by flags: `relatedEdges` (default **on** — the Route 2 relation layer), `entityAlignment` (synonym, default **on**), `coMentionEdges` (default **off**, ablation only), `tripleEdges` (default off, legacy bare-triple graphs). PPR walks **only** entity-entity edges; `MENTIONED_IN` is used separately to project entity scores onto chunks.
...
**PPR graph schema** (Neo4j, scoped by `docName`/corpus): `(:Entity {canonicalId, canonicalName, type, description, aliases})` is the concept layer PPR walks; `(:Chunk)` is the passage layer; relation edges are `RELATED {predicate, description, weight}` (Route 2, the default layer for both arms) and `SYNONYM {cosine}` (identity bridging); `COOCCURS_WITH {count}` (co-mention, ablation-only, never global) and `TRIPLE {count, predicates}` (legacy bare-triples, superseded by `RELATED`) remain loadable; `MENTIONED_IN` (entity→chunk) is projection only.
...
- **Route 2 — LLM (deployed):** one LLM pass extracts entities **+ relationships (with a short predicate) + descriptions** (`graphExtract.py` → `graphBuild.py` → `(:Entity {description})-[:RELATED {predicate, description, weight}]->(:Entity)`). The unified relation edge serves **both arms**: the local arm (PPR) walks `RELATED` as its relation layer, and the global arm (Leiden communities + summaries) clusters it. Uses `llmTriples.callLLM` with the `openrouter` backend (configurable via `GRAPH_EXTRACT_BACKEND`). Route 2 == Stages 5+6 of `docs/GLOBAL_PLAN.md`. **Built.**
...
**Two independent axes (do not conflate):** *construction* (`GRAPH_ROUTE` 1|2 — how the KG is built) vs *retrieval* (local PPR + global GraphRAG, both live, the router picks per query). The arms run on top of whichever graph the route built; the global arm needs descriptions, so it only works on a **Route 2** graph (Route 1 is a local-only baseline).

**Relation edge is unified (Option A):** `RELATED {predicate, description, weight}` is the single relation layer — the local arm (PPR) walks it *and* the global arm (Leiden) clusters it. **Default PPR layers on a Route 2 graph: `RELATED` + `SYNONYM`** (identity bridging, near-always-on). **co-mention is OFF by default** — its v7b benefit was measured on the old bare `TRIPLE` edges, not `RELATED`, so it's an ablation question (`coMentionEdges` toggle in `retrieve.py`) and is **never** fed to the global arm (Leiden = `RELATED`+`SYNONYM` only). **Leiden/global never uses co-mention.**

**Retired but kept on disk as documented prior effort (do not delete):** the ontology-**typed** pipeline, renamed for clarity to `typedOntologyExtract.py` (was `llmExtract.py`) + `typedKgWriter.py` (was `kgWriter.py`), still wired into the old `run.py`/`runAll.py`/`runLlm.py`/`runKg.py`. The bare-**triples** intermediate (`llmTriples` bare prompt + `kgBuild.writeTripleEdges` + `runTripleEdges.py`, PPR v7) is **superseded by Route 2** — `RELATED` carries the predicate it used to, plus a description — *not* retired for being wrong; the relation layer it provided is essential and lives on as `RELATED`. (`llmTriples.callLLM` stays — it is the shared multi-backend LLM entry point.)
